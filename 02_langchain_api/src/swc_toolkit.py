...
try:
    ...
    from swcpy.schemas import League, Team
except ImportError:
    raise ImportError("swcpy가 설치돼있지 않습니다. 설치 후 진행하세요.")
...
class HealthCheckTool(BaseTool):
    ...
    description: str = (
        "다른 API 호출 전에 API가 정상적으로 동작하는지 확인할 때 사용한다."
    )
...
class LeaguesInput(BaseModel):
    league_name: Optional[str] = Field(
        default=None,
        description="리그 이름이다. 전체 리그를 조회하려면 비워 두거나 None으로 설정한다.",
    )
...
class ListLeaguesTool(BaseTool):
    ...
    description: str = (
        "SportsWorldCentral에서 리그 정보를 조회한다."
        "리그에 팀이 있는 경우 함께 반환된다."
    )
    ...
    def _run(
        ...
        """SportsWorldCentral에서 리그 정보를 가져온다."""
        # 리그 이르믕로 API를 호출한다. None도 가능하다.
...
class TeamsInput(BaseModel):
    team_name: Optional[str] = Field(
        default=None,
        description="조회할 팀 이름이다. 전체 팀을 조회하려면 비워 두거나 None으로 설정한다.",
    )
    league_id: Optional[int] = Field(
        default=None,
        description=("리그의 ID(숫자)이다. 전체 리그의 팀을 조회하려면 비워둔다."),
    )
...
class ListTeamsTool(BaseTool):
    name: str = "ListTeams"
    description: str = (
        "SportsWorldCentral에서 팀 정보를 조회한다. 팀 내에 선수가 있는 경우 함께 반환된다. 특정 리그의 팀만 조회하려면 리그 ID를 입력할 수 있다."
    )
    ...
    def _run(
        ...
        """SportsWorldCentral에서 팀 정보를 가져온다."""
